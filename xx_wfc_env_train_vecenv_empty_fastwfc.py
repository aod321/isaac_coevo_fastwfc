...
if __name__ == "__main__":
    time_stmp_str = time.strftime("%Y%m%d-%H%M%S")
    log_dir = f"./training_logs/empty{WFC_SIZE}{WFC_SIZE}_{time_stmp_str}"
    os.makedirs(log_dir, exist_ok=True)
    timesteps = 1500000
    check_freq = 1000
    callback = SaveOnBestTrainingRewardCallback(log_dir=log_dir, check_freq=check_freq)

    m_env = PCGVecEnv(wfc_size=WFC_SIZE,is_node_pairs=ENBALE_NODE_PAIRS,headless_ = True, compute_device_id=1, graphics_device_id=1)
